...
func InsertTestData(db *sqlca.Engine) (err error) {
	...
	for _, user := range users {
		if user.Id == 1 {
			user.LrvTree = sqlca.NewLrvRoot()
		} else {
			admin := &models.User{}
			if _, err = db.Model(&admin).Id(1).Query(); err != nil {
				return log.Errorf("查询管理员数据失败: %s", err)
			}
			log.Infof("管理员：左 %v 右 %v", admin.LeftVal, admin.RightVal)
			user.LrvTree = admin.NewChild(1)
			if err = db.ShiftLRV(user.TableName()); err != nil {
...
func VerifyTestData(db *sqlca.Engine) (err error) {
	...
	if count != 3 {
		return log.Errorf("用户数据数量不正确，期望 3，实际 %d", count)
	}
...
func TestPreload(db *sqlca.Engine) (err error) {
	// 测试关联查询
	var users []*models.User
	rows, err := db.Model(&users).Preload("Roles", "id > ?", 0).Preload("Profile").Limit(2).Query()
	if err != nil {
