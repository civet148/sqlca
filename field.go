...
func parseField(fieldStruct reflect.StructField) *Field {
	...
	field := &Field{
		...
		Updatable:              true,
		Readable:               true,
		PrimaryKey:             checkTruth(tagSetting["PRIMARYKEY"], tagSetting["PRIMARY_KEY"]),
		AutoIncrement:          checkTruth(tagSetting["AUTOINCREMENT"]),
		HasDefaultValue:        checkTruth(tagSetting["AUTOINCREMENT"]),
		NotNull:                checkTruth(tagSetting["NOT NULL"], tagSetting["NOTNULL"]),
		Unique:                 checkTruth(tagSetting["UNIQUE"]),
			...
			getRealFieldValue = func(v reflect.Value) {
				...
				if rv.Kind() == reflect.Struct && !rvType.ConvertibleTo(TimeReflectType) {
					for i := 0; i < rvType.NumField(); i++ {
						...
						for key, value := range parseTagSetting(rvType.Field(i).Tag.Get("sqlca"), ";") {
							if _, ok := field.TagSettings[key]; !ok {
								field.TagSettings[key] = value
							}
						}
	...
	if v, isSerializer := fieldValue.Interface().(SerializerInterface); isSerializer {
		...
		if serializerName != "" {
			if serializer, ok := GetSerializer(serializerName); ok {
				...
				log.Errorf("invalid serializer type %v", serializerName)
			}
		}
	}
	...
	case reflect.Bool:
		...
		if field.HasDefaultValue && !skipParseDefaultValue {
			if field.DefaultValueInterface, err = strconv.ParseBool(field.DefaultValue); err != nil {
				log.Errorf("failed to parse %s as default value for bool, got error: %v", field.DefaultValue, err)
			}
		}
	case reflect.Int, reflect.Int8, reflect.Int16, reflect.Int32, reflect.Int64:
		...
		if field.HasDefaultValue && !skipParseDefaultValue {
			if field.DefaultValueInterface, err = strconv.ParseInt(field.DefaultValue, 0, 64); err != nil {
				log.Errorf("failed to parse %s as default value for int, got error: %v", field.DefaultValue, err)
			}
		}
	case reflect.Uint, reflect.Uint8, reflect.Uint16, reflect.Uint32, reflect.Uint64:
		...
		if field.HasDefaultValue && !skipParseDefaultValue {
			if field.DefaultValueInterface, err = strconv.ParseUint(field.DefaultValue, 0, 64); err != nil {
				log.Errorf("failed to parse %s as default value for uint, got error: %v", field.DefaultValue, err)
			}
		}
	case reflect.Float32, reflect.Float64:
		...
		if field.HasDefaultValue && !skipParseDefaultValue {
			if field.DefaultValueInterface, err = strconv.ParseFloat(field.DefaultValue, 64); err != nil {
				log.Errorf("failed to parse %s as default value for float, got error: %v", field.DefaultValue, err)
			}
		}
	case reflect.String:
		field.DataType = String
	...
	if v, ok := field.TagSettings["AUTOCREATETIME"]; (ok && checkTruth(v)) || (!ok && field.Name == "CreatedAt" && (field.DataType == Time || field.DataType == Int || field.DataType == Uint)) {
	...
	if v, ok := field.TagSettings["AUTOUPDATETIME"]; (ok && checkTruth(v)) || (!ok && field.Name == "UpdatedAt" && (field.DataType == Time || field.DataType == Int || field.DataType == Uint)) {
		if field.DataType == Time {
	...
	case reflect.String:
		field.Set = func(ctx context.Context, value reflect.Value, v interface{}) (err error) {
			...
			case int, int8, int16, int32, int64, uint, uint8, uint16, uint32, uint64:
				field.ReflectValueOf(ctx, value).SetString(toString(data))
